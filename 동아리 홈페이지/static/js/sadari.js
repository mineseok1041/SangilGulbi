...
function generateLadder() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        resultText.textContent = '결과'; // 기본 텍스트
        resultText.className = 'result-text'; // 클래스 추가
        line.appendChild(resultText); // 세로줄 하단에 결과 추가
    }
    ...
    for (let i = 0; i < count - 1; i++) {
        ...
        for (let j = 0; j < numLines; j++) { 
            const position = Math.random() * (550 - (lineHeight * (count - 1) + 10 * (numLines - 1))) + (20 + (lineHeight * j) + 10 * j); // 수평선 위치 계산
            ...
            // 3번째 가로줄부터 5픽셀 오른쪽으로 이동
            if (i >= 2) { 
                horizontal.style.left = `${70 + i * 60}px`; // 3번째부터 오른쪽으로 이동
            } else {
                horizontal.style.left = `${57 + i * 60}px`; // 기본 위치
            }
...
function startGame() {
    ...
    // 각 세로줄 하단에 미리 입력한 결과 표시
    for (let i = 0; i < count; i++) {
        const resultText = document.querySelectorAll('.result-text')[i]; // 각 세로줄의 결과 텍스트 선택
        resultText.textContent = results[i]; // 결과로 업데이트
    }
}
...
function moveBall(ball, index) {
    ...
    const interval = setInterval(() => {
        ...
        if (y >= 600) { // 높이에 맞춰 조정
            ...
            alert(`${players[index]}의 결과: ${results[x]}`); // 결과 호출
        }
    }, 100);
}
